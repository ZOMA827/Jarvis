...
def init_gemini_chat():
    ...
    jarvis_instructions = (
        "You are JRAVIST, an advanced holographic AI operating system core. "
    ...
    # تفعيل ميزة استدعاء الدوال التلقائي (هنا السحر!)
    chat = model.start_chat(enable_automatic_function_calling=True)
    print(f"🤖 JRAVIST Core initialized with all Holographic Tools using Key #{current_key_index + 1}")


   

# تشغيل التهيئة الأولية عند إقلاع البرنامج
init_gemini_chat()
...
def takeAllCommands(message=None):
    try:
        ...
        # 2. بوابة التفتيش الذكية: لحماية الرصيد
            if query == "none" or query.strip() == "":
                return 
        
        # 3. إذا كان هناك كلام حقيقي، نقوم بمعالجته وإرساله
            print(f"User said: {query}")
        ...
        # ... (باقي الكود الخاص بحلقة التبديل for كما هو عندك) ...
