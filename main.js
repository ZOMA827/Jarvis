...
$(document).ready(function () {
  /* ── فحص وتأمين نظام المفاتيح عند إقلاع الواجهة ── */
  eel.check_api_keys_exist()(function(hasKeys) {
    if (!hasKeys) {
      // إظهار نافذة إدخال المفاتيح (Modal) ومنع المستخدم من التفاعل مع جارفيس حتى يضع مفتاحاً واحداً على الأقل
      $("#KeySetupModal").css("display", "flex");
    }
  });

  /* ── زر حفظ وتفعيل النظام ── */
  $("#SaveKeysBtn").click(function() {
    var keys = [];
    ...
    if (keys.length === 0) {
      $("#SetupStatus").text("⚠️ ERROR: PLEASE ENTER AT LEAST ONE API KEY.");
      return;
    }

    $("#SetupStatus").text("⚡ INJECTING SECURE CRYPTO KEYS...");

    eel.save_user_keys(keys)(function(success) {
      if (success) {
        $("#SetupStatus").text("✅ CORE SYSTEMS ONLINE. STARTING JARVIS...");
        setTimeout(function() {
          $("#KeySetupModal").fadeOut(500); // إخفاء النافذة والدخول للنظام الأساسي
        }, 1500);
      } else {
        $("#SetupStatus").text("❌ CRITICAL ERROR: INITIALIZATION FAILED.");
      }
    });
  });
  /* ── Init Eel ─────────────────────────────────── */
